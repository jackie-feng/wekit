...
const PAGE_EVENT = ['onLoad', 'onReady', 'onShow', 'onHide', 'onUnload', 'onPullDownRefresh', 'onReachBottom', 'onShareAppMessage','onPageScroll'];
...
let $bindEvt = (config, com, prefix) => {
    ...
    // console.log('props $bindEvt', config, com, prefix)
    com.$prefix = util.camelize(prefix || '');
    Object.getOwnPropertyNames(com.components || {}).forEach((name) => {
        let cClass = com.components[name];
        // console.log('info $bindEvt child name=', name, cClass);
        let child;
        if (typeof (cClass) === 'object') {
            child = new Component();
            child.$bindPageObject(cClass);
        } else {
            child = new cClass();
        }
        child.$initMixins();
    ...
    allMethods.forEach((method, i) => {
        ...
        config[com.$prefix + method] = function (e, ...args) {
            ...
            // console.log('props comIndex', comIndex)
            ...
            if (comIndex !== undefined) {
                ...
                while (level-- > 0) {
                    ...
                    // console.log('props tmpcom', tmpcom.$prefix)
...
let $bindMethod = (config, vue) => {
    let methods = vue.$options.methods || {};

    let prefix = vue.$prefix
    Object.keys(methods).forEach(name => {

        let methodName = prefix? prefix+'$'+name : name;

        config[methodName] = function (e, ...args) {
            // console.log('props config', methodName)

            // console.log('methodName=',methodName,e,args)
            ...
            if (comfn) {
                rst = comfn.apply(vue, args);
            }
            return rst
        }
    });

    Object.keys(vue._childs || {}).forEach(name => {
        let child = vue._childs[name];
        $bindMethod(config,child);
    })
...
export default {
    ...
    $createPage(pageClass, pagePath) {
        ...
        PAGE_EVENT.forEach(v=>{
            config[v] = function (...args) {
                ...
                if (v === 'onLoad') {
                    ...
                    pageClass.wxpage = this;
                    page = new Vue(pageClass);

                }


                let method = page[v] || page.$options[v];
                if (method) {
                    res= method.apply(page, args);
                }
                return res;
            }
        });

        if (pageClass['onShareAppMessage'] === null) {
            delete config['onShareAppMessage'];
        }

        // $bindMethod(config,page);
        ...
        //   // console.log('### config onload args=',args);
        ...
        //   // });
        //
        //   // console.log('page', 'createPage', page)
        //
        //   page.$apply();
        // };
        //
