...
export default {
    resolveDeps (code, type, opath) {
        ...
        util.Log('resolveDeps,opath=',opath.base);
        ...
        return code.replace(/require\(['"]([\w\d_\-\.\/@]+)['"]\)/ig, (match, lib) => {
            ...
            //处理ocomp组件规则
            const ocomp_path = util.oCompRealPath(source);
            if (ocomp_path) {
                // console.log("ocomp_path:", ocomp_path)
                resolved = ocomp_path.replace(/\.vue/i, ".js")
                resolved = path.relative(opath.dir, resolved)
            } 
    ...
    compile (lang, code, type, opath) {
        ...
        // console.log('编译js:', opath.base);
        //todo: 不要编译组件index.js
        
        code = code.replace(/暴露[\n\s\r]*?import { register } from 'utils'/g, '')
	
		if (opath.base.match(/\.json$/)) {
			util.Log('json', opath);
        ...
        compiler(code, config.compilers[lang] || {}).then(compileResult => {
            // console.log("compileResult for ", opath)
            let sourceMap;
            ...
            util.Log('opath ,type',opath.base,type,type==='page');
            if (type !== 'npm') {
                if (type === 'page' || type === 'app') {
                    code = code.replace(/(export\s+default\s+|module\.exports\s+=\s+|exports\.default\s+=\s+){((\n|.)*)}/g, function (m, exports, code) {
                        let s = `var $export$${opath.name} = {${code}}`

						// util.Log('exports', opath.name, exports, code)
						// console.log('xxopath1',opath.base);
						if (type === 'page') {
							let pagePath = path.join(path.relative(appPath.dir, opath.dir), opath.name).replace(/\\/ig, '/');
                            s = s + `\nPage(require('wekit').default.$createPage($export$${opath.name} , '${pagePath}'));\n`;
						} else {
							appPath = opath;
							let appConfig = JSON.stringify(config.appConfig);
                            s = s + `\nApp(require('wekit').default.$createApp( $export$${opath.name}, ${appConfig}));\n`;
						}s
                    ...
                    code = code.replace(/exports\.default\s*=\s*(\w+);/ig, function (m, defaultExport) {
						// util.Log('opath',code,type, opath.dir, opath.base)
                        // util.Log(m, defaultExport)
                        if (defaultExport === 'undefined') {
                            return '';
                        }
                        if (type === 'page') {
                            let pagePath = path.join(path.relative(appPath.dir, opath.dir), opath.name).replace(/\\/ig, '/');
                            return `\nPage(require('wekit').default.$createPage(${defaultExport} , '${pagePath}'));\n`;
                        } else {
                            appPath = opath;
                            let appConfig = JSON.stringify(config.appConfig);
                            return `\nApp(require('wekit').default.$createApp(${defaultExport}, ${appConfig}));\n`;
                        }
                    });
                }
            }

            // util.Log('resolveDeps, code=',code);
			...
			if (opath.base.match(/\.json$/)) {
				target = path.join(opath.dir, opath.base + '.js');
            ...
            util.error('compile-script.js:'+ e + e.stack + '\n 源码:' + code);
            process.exit(0);
        });
    }
