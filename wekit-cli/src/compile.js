...
export default {
     ...
     */
    findReference (file) {
        console.log("findReference", file)
        let src = cache.getSrc();
        let files = util.getFiles(src);
        let ext = cache.getExt();
    ...
    wekitUpdate(required = '1.2.0') {
        let o = resolve.getPkg('wekit') || {};
    ...
    init (config) {
        let wepyrc = util.getConfig();
        if (!wepyrc) {
            util.error('没有检测到wekit.config.js文件, 请执行`wekit new demo`创建');
            return false;
        }
        resolve.init(wepyrc.resolve || {});
        loader.attach(resolve);

        if (this.wekitUpdate()) { // 需要更新wepy版本
            util.log('检测到wekit版本不符合要求，正在尝试更新，请稍等。', '信息');
            util.exec(`npm install wekit --save`).then(d => {
                util.log(`已完成更新，重新启动编译。`, '完成');
                this.build(config);
            }).catch(e => {
                util.log(`安装wekit失败，请尝试运行命令 "npm install wekit --save" 进行安装。`, '错误');
                console.log(e);
            });
            return false;
        }
        ...
        if (config.output === 'web') {
            ...
            files.forEach((f) => {
                ...
                if (file) {
                    ...
                    if (!refs.length) {
                        this.compile(opath);
                        // console.log(`compile ${opath.base} finish`)
                    }
                }
            });
        }
